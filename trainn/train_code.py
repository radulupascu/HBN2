...
new_rows = pd.DataFrame({
    'product_name': ['None'] * 100,
    'category_name': ['None'] * 100,
    'family_code': ["None"] * 100
})

# Append the new DataFrame to the existing DataFrame
df = pd.concat([df, new_rows], ignore_index=True)

# Display the updated dataframe shape to confirm the addition
print(df.shape)
...
df.to_csv("cleaned_dataset_str.csv", index=False)
...
def add_labels(examples):
    ...
    examples['labels'] = examples['family_code_encoded']  # or 'family_code_encoded' for the other model
    return examples
...
num_labels = df['family_code_encoded'].nunique()  # or df['family_code_encoded'].nunique() for codes
...
trainer.model.save_pretrained('./saved_model_family_code')
tokenizer.save_pretrained('./saved_model_family_code')
