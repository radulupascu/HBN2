...
unique_labels = labels_df['family_code'].unique()
...
def predict(df, model, tokenizer, label_encoder):
    ...
    df['predicted_family_code'] = predicted_labels
    return df

# Load the model and tokenizer
model = BertForSequenceClassification.from_pretrained('./saved_model_family_code')
tokenizer = BertTokenizer.from_pretrained('./saved_model_family_code')
...
df.to_csv("prediction_code.csv", mode='a', header=False, index=False)
